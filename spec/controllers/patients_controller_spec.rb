...
RSpec.describe PatientsController, type: :controller do
  ...
  describe 'POST #create' do
    ...
    context 'when the user is an admin' do
      ...
      it 'transfers the form to a new user if nok_email is present' do
        form_params = attributes_for(:form, first_name: 'John', last_name: 'Doe', nok_email: 'testing123@gmail.com', start_date: Date.today)
        post :create, params: { form: form_params, commit: 'Save' }
        expect(Form.last.user).to eq(user)
        expect(response).to redirect_to(edit_1_form_path(Form.last))
      end
    end
  end
  ...
  before do
    ...
    context 'when moving to next step' do
      ...
      specify 'if no valid action, redirect to edit_2_forms_path without saving form' do 
        patch :update_1, params: {id: @form.id, form: { first_name: 'Johnathan' }, commit: '' }
        expect(@form.reload.first_name).to eq('John')
        expect(response).to render_template(:edit_1)
      end
    end
  end
  ...
  before { get :edit_2, params: { id: form.id } }
    ...
    it 'renders the edit_2 template' do
  ...
  before do 
    form_params = attributes_for(:form, first_name: 'John', last_name: 'Doe', height: 123, weight: 123)
    post :create, params: { form: form_params, commit: 'Save' }
    expect(Form.count).to eq(1)
    @form = Form.last
  end
  ...
  before { get :edit_3, params: { id: form.id } }
    ...
    it 'renders the edit_3 template' do
  ...
  before do 
    form_params = attributes_for(:form, first_name: 'John', last_name: 'Doe', height: 123, weight: 123, services: ["companionship"], start_date: Date.today + 2.days)
    post :create, params: { form: form_params, commit: 'Save' }
    expect(Form.count).to eq(1)
    @form = Form.last
  end
  ...
  before { get :edit_4, params: { id: form.id } }
    ...
    it 'renders the edit_4 template' do
  ...
  before do 
    form_params = attributes_for(:form, first_name: 'John', last_name: 'Doe', height: 123, weight: 123, services: ["companionship"], start_date: Date.today + 2.days, physical_video_file_name: "HELLO.mp4")
    post :create, params: { form: form_params, commit: 'Save' }
    expect(Form.count).to eq(1)
    @form = Form.last
  end
    context 'when uploading a physical video' do
      it 'updates the physical video file name and returns success' do
  ...
  before do
    ...
    context 'when form creation fails' do
      before do
        #it will return a new Form object without actually creating a record in the database so it means that creation of form failed
  ...
  let!(:form) { create(:form, user: user) }
  before { get :edit_5, params: { id: form.id } }

    it 'assigns the form' do
      expect(assigns(:form)).to eq(form)
    end

    it 'renders the edit_5 template' do
      expect(response).to render_template(:edit_5)
    end
  end

  describe 'PATCH #update_5' do
  before do 
    form_params = attributes_for(:form, first_name: 'John', last_name: 'Doe', height: 123, weight: 123, services: ["companionship"], start_date: Date.today + 2.days, physical_video_file_name: "HELLO.mp4", environment_video_file_name: "TEST.mp4")
    post :create, params: { form: form_params, commit: 'Save' }
    expect(Form.count).to eq(1)
    @form = Form.last
  end
  ...
  before {get :show, params: { id: form.id }}
    context 'when requesting a specific form id' do
      it 'assigns the requested form to that form' do
        expect(assigns(:form)).to eq(form)
      end
  ...
  before do 
    ...
    context 'when user is an admin' do
      ...
      it 'deletes the form if form is not submitted' do
    ...
    context 'when user is a user' do
      ...
      it 'deletes the form if not submitted' do
        delete :destroy, params: { id: @form.id }
        expect(Form.exists?(@form.id)).to be_falsey
      end
      it 'does not deletes the form if submitted' do
        @form.update(submitted: true)
        delete :destroy, params: { id: @form.id }
        expect(Form.exists?(@form.id)).to be_truthy
      end
    end
  end
  ...
  describe 'GET #show_error' do
    context 'no form id' do     
      it 'renders the :show error template' do
        get :show_error
